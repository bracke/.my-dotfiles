...
bash_prompt_command
 ...
 ############################################################################
PS1="$TITLEBAR\n${PROMT_USER}\D{%T}${SEPARATOR_1}${PROMT_PWD}${SEPARATOR_3}${PROMT_INPUT}"

if [[ $(find .git 2> /dev/null | wc -l) > 0 ]]; then

        local branch="$(git branch | grep '*' | cut -d ' ' -f 2)"
        local remote="$(git remote)/$branch"
        local status=""

        # Commits ahead
        local ahead=$(git rev-list --count $remote..$branch)
        [[ $ahead > 0 ]] && status="$status ↑$ahead"

        # Commits behind
        local behind=$(git rev-list --count $branch..$remote)
        [[ $behind > 0 ]] && status="$status ↓$behind"

        # Added files
        A=$(git status -s | grep -Pc "^[RAM]..")
        [[ $A > 0 ]] && status="$status +$A"

        # Modifiled files
        M=$(git status -s | grep -Pc "^.M.")
        [[ $M > 0 ]] && status="$status ~$M"

        # Untracked files
        U=$(git status -s | grep -c "?? ")
        [[ $U > 0 ]] && status="$status U$U"

        # Deleted files
        D=$(git status -s | grep -Pc "^.D.")
        [[ $D > 0 ]] && status="$status -$D"

	PS1="$TITLEBAR\n${PROMT_USER}\D{%T}${SEPARATOR_1}${PROMT_PWD} ${branch}${status}${SEPARATOR_3}${PROMT_INPUT}"
fi
...
fortune de
echo
curl -H "Accept-Language: da" Wttr.in/Ikast?0M
echo
alias config='/usr/bin/git --git-dir=/home/bent/.my-dotfiles/ --work-tree=/home/bent'
